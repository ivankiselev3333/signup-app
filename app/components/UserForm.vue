  ...
  setup(props) {
    ...
    const rules = reactive({
      ...
      email: [
        ...
        {
          pattern: /^[^@\s]+@[^@\s]+\.[^@\s]+$/,
          message: 'Please provide valid email address, valid email example somename@mail.ru',
          trigger: 'blur'
        },
      ],
    });
          ...
          .then(
              async () => {
                ...
                const updatedUser = !!userId ?
                    await api.helpPatch(`users/${userId}`, user) :
                    await api.helpPost('users', user);
                Object.assign(user, updatedUser);
                router.push({name: 'user-display', params: {userId: user.id}});
              }
          )
          .catch(() => {
          });
